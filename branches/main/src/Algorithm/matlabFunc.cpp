...
CMatlabFunc::erodeY( CFrameContainer* const pFrame_in, uint32_t nErodeTimesPerPixel_in ) const
{
  ...
  uint16_t const nWidth=pFrame_in->getWidth();//图像的宽
  uint16_t const nHeight=pFrame_in->getHeight();//图像高
...
{ 
  ...
  int m,n,i=0,j=0;
  uint32_t k=0;
  uint16_t nWidth=pFrame_in->getWidth();//图像的宽
  uint16_t nHeight=pFrame_in->getHeight();//图像高
  uint8_t mpiex,mpiex1;
  bool flag=FALSE;
  uint8_t *pData = new uint8_t[nWidth*nHeight];
