...
void CSettingDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
  ...
  {
    if(point.x>line_TestGUI->pos_pic.x+line_TestGUI->Pic_sizex||point.y>line_TestGUI->pos_pic.y+line_TestGUI->Pic_sizey||point.x<line_TestGUI->pos_pic.x||point.y<line_TestGUI->pos_pic.y)
    {
      return;
    }
